...
class PascalVocWriter:
    ...
    def addBndBox(self, xmin, ymin, xmax, ymax, name, occluded):
        bndbox = {'xmin': xmin, 'ymin': ymin, 'xmax': xmax, 'ymax': ymax}
        bndbox['name'] = name
        bndbox['occluded'] = occluded
        if ((xmax-xmin)>25 and (ymax-ymin)>25): #here
...
class PascalVocReader:
    ...
    def __init__(self, filepath):
        # shapes type:
        # [labbel, [(x1,y1), (x2,y2), (x3,y3), (x4,y4)], color, color, occluded]
        self.shapes = []
    ...
    def addShape(self, label, bndbox, occluded):
        xmin = int(bndbox.find('xmin').text)
        ymin = int(bndbox.find('ymin').text)
        xmax = int(bndbox.find('xmax').text)
        ymax = int(bndbox.find('ymax').text)
        points = [(xmin, ymin), (xmax, ymin), (xmax, ymax), (xmin, ymax)]
        self.shapes.append((label, points, None, None, occluded))
    ...
    def parseXML(self):
        ...
        for object_iter in xmltree.findall('object'):
            ...
            self.addShape(label, bndbox, occluded)
        return True
